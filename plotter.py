...
def create_hourly_line_plot(hr: pd.DataFrame, title: str = "Avg Infusions by Hour of Session"):
    """
    Mean infusion events per hour, correctly averaged over ALL sessions
    (including sessions where that hour had zero events).

    Uses session_day (integer, no NaT) as the session counter when available,
    falling back to start_date.  This gives the correct denominator even when
    some start_date values are NaT.
    """
    if hr.empty or "infusion_events" not in hr.columns:
        return go.Figure().update_layout(title=f"{title} — No Data")

    # Denominator: total unique sessions per subject
    if "session_day" in hr.columns:
        n_sessions = (
        ...
        # Last resort: count rows / max_hour+1 — imprecise but won't crash
        n_sessions = (
...
def create_efficiency_trend(daily: pd.DataFrame):
    ...
    return create_interactive_plot(df, date_col, "efficiency",
...
def create_hourly_heatmap(hr: pd.DataFrame):
    ...
    fig = px.imshow(pivot, title="Hourly Infusion Heatmap",
