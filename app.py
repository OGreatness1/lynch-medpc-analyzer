...
if ("df_sess" in st.session_state and st.session_state.df_sess is not None and not st.session_state.df_sess.empty):
    ...
    if df_sess.empty:
        ...
        # ─── SMART GENDER CHECKER ───
        unique_genders = df_sess["gender"].unique()
        if "Male" not in unique_genders or "Female" not in unique_genders:
            st.info("💡 **Gender Comparisons Hidden:** The app automatically compares cohorts by gender if it detects both 'Male' and 'Female' subjects. It does this by checking the *last letter* of your Subject IDs (e.g., '123M' or '123F'). Your current subjects were marked as 'Unknown', so these plots are hidden.")
        ...
        with tab_cohort:
            ...
            for tab_idx, (tab, p) in enumerate(zip(prog_tabs, programs)):
                with tab:
                    ...
                    with c1:
                        if not daily.empty:
                            st.plotly_chart(
                                ...
                                use_container_width=True, key=f"daily_inf_{p}_{tab_idx}")
                            st.plotly_chart(create_mean_sem_trajectory(daily), use_container_width=True, key=f"mean_sem_{p}_{tab_idx}")
                    with c2:
                        if not sub_h.empty:
                    ...
                    st.divider()
                    st.plotly_chart(create_cohort_discrimination_plot(sub_s), use_container_width=True, key=f"cohort_discrim_{p}_{tab_idx}")
                    ...
                    if has_both:
                        ...
                        with gc1:
                            if not daily.empty:
                                st.plotly_chart(create_mean_sem_trajectory(daily, split_by_gender=True), use_container_width=True, key=f"mean_sem_gen_{p}_{tab_idx}")
                            st.plotly_chart(create_cohort_discrimination_plot(sub_s, split_by_gender=True), use_container_width=True, key=f"cohort_discrim_gen_{p}_{tab_idx}")
                        with gc2:
                            if not sub_h.empty:
                                st.plotly_chart(create_cohort_hourly_line_plot(sub_h, split_by_gender=True), use_container_width=True, key=f"cohort_hourly_gen_{p}_{tab_idx}")

        with tab_advanced:
            has_both_genders_all = "Male" in df_sess["gender"].values and "Female" in df_sess["gender"].values
                        ...
                        for tab_idx, (tab, p) in enumerate(zip(prog_tabs, programs)):
                            with tab:
                                ...
                                if not prog_daily.empty:
                                    st.plotly_chart(
                                        create_interactive_plot(
                                            ...
                                            f"Daily Infusions — {p} ({sel})", hue=None, kind="line"),
                                        use_container_width=True, key=f"daily_inf_subj_{sel}_{p}_{tab_idx}")

                                if not prog_hr.empty:
                                    st.plotly_chart(create_hourly_line_plot(prog_hr), use_container_width=True, key=f"hourly_line_subj_{sel}_{p}_{tab_idx}")
                                    st.plotly_chart(create_hourly_heatmap(prog_hr), use_container_width=True, key=f"hourly_heatmap_subj_{sel}_{p}_{tab_idx}")

                                st.subheader(f"Additional Plots — {p}")

                                if not prog_sess.empty:
                                    st.plotly_chart(create_cumulative_plot(prog_sess), use_container_width=True, key=f"cumulative_subj_{sel}_{p}_{tab_idx}")

                                if not prog_sess.empty and "active_presses" in prog_sess.columns:
                                    st.plotly_chart(create_discrimination_plot(prog_sess), use_container_width=True, key=f"discrim_subj_{sel}_{p}_{tab_idx}")
                                    st.plotly_chart(create_response_rate_plot(prog_sess), use_container_width=True, key=f"resp_rate_subj_{sel}_{p}_{tab_idx}")

                                if (not prog_sess.empty and "breakpoints" in prog_sess.columns and prog_sess["breakpoints"].sum() > 0):
                                    st.plotly_chart(create_pr_breakpoint_plot(prog_sess), use_container_width=True, key=f"pr_bp_subj_{sel}_{p}_{tab_idx}")

                                if not prog_daily.empty and "total_active_presses" in prog_daily.columns:
                                    st.plotly_chart(create_efficiency_trend(prog_daily), use_container_width=True, key=f"efficiency_subj_{sel}_{p}_{tab_idx}")
                                ...
                                if ("active_timestamps" in prog_sess.columns and "session_day" in prog_sess.columns):
                                    ...
                                    selected_label = st.selectbox("Select session to view timepoints:", options=list(session_options.keys()), key=f"ts_sel_{sel}_{p}_{tab_idx}")
                                    if selected_label:
                                        ...
                                        st.plotly_chart(create_within_session_plot(timestamps, duration), use_container_width=True, key=f"ts_plot_{sel}_{p}_{tab_idx}")
                                else:
